...
export function registerCollectionTools(
  ...
  if (isToolEnabled("ptcgp_collection_stats", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_missing_cards", options))
    server.registerTool(
      "ptcgp_missing_cards",
      {
...
Devuelve { packs: [{pack, expansion, missing_count, total_in_pack, missing: [{id, name, rarity}] }] }. Con include_cards=false omite el listado de cartas y devuelve solo el ranking (más compacto).`,
  ...
  if (isToolEnabled("ptcgp_set_card_quantity", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_bulk_update_collection", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_mark_range", options))
    server.registerTool(
      "ptcgp_mark_range",
      {
