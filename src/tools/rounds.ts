...
import { isToolEnabled, type RegistrarOptions } from "../server/factory.js";
...
export function registerRoundTools(
  server: McpServer,
  options?: RegistrarOptions,
): void {
  if (isToolEnabled("ptcgp_round_start", options))
    server.registerTool(
      "ptcgp_round_start",
      {
        ...
        inputSchema: {
          ...
          expected_owned_unique: z
            ...
            .optional()
            .describe(
              "Total de cartas únicas poseídas mostrado en la cabecera",
            ),
  ...
  if (isToolEnabled("ptcgp_round_analyze_screenshots", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_round_record", options))
    server.registerTool(
      "ptcgp_round_record",
      {
  ...
  if (isToolEnabled("ptcgp_round_status", options))
    server.registerTool(
      "ptcgp_round_status",
      {
  ...
  if (isToolEnabled("ptcgp_round_finalize", options))
    server.registerTool(
