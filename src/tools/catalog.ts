...
export function registerCatalogTools(
  ...
  if (isToolEnabled("ptcgp_search_cards", options))
    server.registerTool(
      "ptcgp_search_cards",
      {
        title: "Buscar cartas",
...
Ejemplos: "atacantes de fuego para mi mazo que ya tenga" -> type="Fire", min_damage=100, owned_filter="owned". "cartas que curen" -> text_search="heal".`,
  ...
  if (isToolEnabled("ptcgp_get_card", options))
    server.registerTool(
      "ptcgp_get_card",
      {
  ...
  if (isToolEnabled("ptcgp_enrich_catalog", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_list_expansions", options))
    server.registerTool(
  ...
  if (isToolEnabled("ptcgp_sync_catalog", options))
    server.registerTool(
      "ptcgp_sync_catalog",
      {
