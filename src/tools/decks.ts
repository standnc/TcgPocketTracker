...
export function registerDeckTools(
  server: McpServer,
  options?: RegistrarOptions,
): void {
  if (isToolEnabled("ptcgp_meta_decks", options))
    server.registerTool(
      "ptcgp_meta_decks",
      {
  ...
  if (isToolEnabled("ptcgp_get_decklist", options))
    server.registerTool(
      "ptcgp_get_decklist",
      {
...
Ejemplo de flujo: ptcgp_meta_decks -> elegir slug -> ptcgp_get_decklist(slug) -> "te faltan 2 Espeon (b3a-020) y 1 Cyrus (a2-150)". Requiere red.`,
        inputSchema: {
          slug: z
            ...
            .max(120)
            .describe(
              "Slug del arquetipo, ej. 'mega-altaria-ex-b1-espeon-b3a'",
            ),
          finish_index: z
            .number()
            .int()
