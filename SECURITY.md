...
- **Rate limit**: el server aplica una ventana fija en memoria por IP antes de autenticar (60 peticiones/minuto/IP por defecto), con un máximo de claves para impedir crecimiento sin límite. Caddy limpia y reescribe `X-Forwarded-For` con la IP remota antes de enviar al proceso loopback. Si se escala a varias instancias, se necesita un limitador compartido en el proxy o Redis.
